...
do
    case ${opt} in
        ...
        ;;
        ?)
        echo -e "\033[32mUsage: \n接口地址设置为host，运行任务ID为taskid的接口用例:\033[0m\n\033[34m./run.sh -h 'host地址' -t 'taskid'\033[0m\n"
        echo -e "\033[32m接口地址设置为host，运行所有接口用例:\033[0m\n\033[34m./run.sh -h 'host地址'\033[0m\n"
        echo -e "\033[32m接口地址使用默认值，运行任务ID为taskid的接口用例:\033[0m\n\033[34m./run.sh -t 'taskid'\033[0m\n"
...
if [[ ${host} ]]; then
    python3 ./scripts/saveconfig.py 1 ${service_url}
else
    python3 ./scripts/saveconfig.py 0
fi
...
if [[ ${task_id} ]]; then
		python3 ../lib/pytest.py test_"${task_id}".py -o log_cli=true -o log_cli_level=INFO --alluredir=../report
		status_code=$?
else
		python3 ../lib/pytest.py --alluredir=../report
		status_code=$?
fi
